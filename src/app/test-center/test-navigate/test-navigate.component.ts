...
export class TestNavigateComponent implements OnInit {
    ...
    handleNextQuestion(): void {
        if (this.order === this.questionCodes.length) { return; }
        this.order += 1;
        this.changeQuestionByOrder(this.order);
    }
    ...
    changeDistance(): void {
        ...
        if (this.order === 1) { change = this.order - 1; }
        if (this.order === this.questionCodes.length) { change = this.order - 3; }
        this.distance = -change * this.unit;
    }

    getStateOrderChange(): string {
        if (!this.checkNavigateChange()) { return this.actionNavigate; }
